...
export const Publications = () => {
  ...
  const publications: Publication[] = [
    ...
    },
    {
      id: 2,
      type: 'article',
      link: "/ARTICLE SANCTIONS CIBLEES REGIS KATUALA.pdf",
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredPublications.map((pub) => (
            <div key={pub.id} className="bg-slate-50 p-6 rounded-lg shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
              ...
              <a href={pub.link} download className=' inline-block px-2 py-1 my-4 bg-amber-600 text-white rounded-lg shadow hover:bg-amber-700 transition-colors'>
                Télécharger
              </a>
            </div>
          ))}
